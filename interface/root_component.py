...
# The same logger object as the one configured in main.py
...
class Root(tk.Tk):
    def __init__(self, binance: BinanceClient, bitmex: BitmexClient):
        ...
        self.title("Kame Trading Bot")
        self.protocol("WM_DELETE_WINDOW", self._ask_before_close)

        self.configure(bg=BG_COLOR)

        # Create the menu, sub menu, and menu commands
        # Can be used when you don't want to load the interface with too many buttons
        ...
        # Seperate the root component in two blocks
        ...
        # Create and place components at the top and bottom of the left and right frames
        ...
        # Space a bit the component with vertical padding
        ...
        # Start the infinite interface update loop
        self._update_ui()
    ...
    def _ask_before_close(self):
        ...
        if result == "yes":
            # Avoid the infinite reconnect loop in _start_ws()
            self.binance.reconnect = False
            self.bitmex.reconnect = False
            self.binance.ws.close()
            self.bitmex.ws.close()

            # Destroy the UI and terminate the program as no other thread is running
            self.destroy()

    # Called by itself every 1500 seconds.
    def _update_ui(self):
        ...
        for client in [self.binance, self.bitmex]:
            ...
            # Handle the case when a dictionary is updated during the following loops
            try:
                for b_index, strat in client.strategies.items():
                    ...
                    # Update the Trades component (add new trade, change status/PNL)
                    for trade in strat.trades:
                        ...
                        if "binance" in trade.contract.exchange:
                            ...
                            # Bitmex PNL is always in BTC, thus 8 decimals
                            precision = 8
    ...
    # Collect the current data on the interface and save it to the SQLite database
    def _save_workspace(self):
        ...
        # Loop through the rows of a column (not necessarily the 'contract' one)
        for b_index in strat_widgets['contract']:
            ...
            # Extra parameters are all saved in one column as a JSON string because they changed based on the strategy
            extra_params = dict()
