...
class StrategyEditor(tk.Frame):
    ...
    def __init__(self, root: "Root", binance: BinanceClient, bitmex: BitmexClient, *args, **kwargs):
        ...
        self._valid_int = self.register(check_int_format)
        ...
        for exchange, client in self._exchanges.items():
            for symbol, contract in client.contracts.items():
                # If you want less symbols in the list, filter here
        ...
        # Define the widgets displayed on each row and some characteristics of these widgets like their width.
        # This lets the program create the widgets dynamically.
        ...
        self._base_params = [
            ...
            {"code_name": "parameters", "widget": tk.Button, "data_type": float, "text": "Parameters",
             "bg": BG_COLOR_2, "command": self._show_popup, "width": 10, "header": ""},
            {"code_name": "activation", "widget": tk.Button, "data_type": float, "text": "OFF",
             "bg": "darkred", "command": self._switch_strategy, "width": 8, "header": ""},
            {"code_name": "delete", "widget": tk.Button, "data_type": float, "text": "X",
             "bg": "darkred", "command": self._delete_row, "width": 6, "header": ""}
        ]
        ...
        header = tk.Label(self._headers_frame, text="", bg=BG_COLOR, fg=FG_COLOR, font=GLOBAL_FONT, width=8, bd=1,
                          relief=tk.FLAT)
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                self.body_widgets[code_name][b_index] = tk.Entry(self._body_frame.sub_frame, justify=tk.CENTER,
                                                                 bg=BG_COLOR_2, fg=FG_COLOR, font=GLOBAL_FONT, bd=1,
                                                                 width=base_param['width'])

                if base_param['data_type'] == int:
                    self.body_widgets[code_name][b_index].config(validate='key', validatecommand=(self._valid_int,
                                                                                                  "%P"))

                elif base_param['data_type'] == float:
                    self.body_widgets[code_name][b_index].config(validate='key', validatecommand=(self._valid_float,
                                                                                                  "%P"))
    ...
    def _show_popup(self, b_index: int):
        ...
        for param in self.extra_params[strat_selected]:
            ...
            if param['widget'] == tk.Entry:
                ...
                # Set the data validation function based on the data_type chosen
                if param['data_type'] == int:
                    self._extra_input[code_name].config(validate='key', validatecommand=(self._valid_int, "%P"))
    ...
    def _switch_strategy(self, b_index: int):
        ...
        if self.body_widgets['activation'][b_index].cget("text") == "OFF":
            ...
            # Collect historical data. It is just one API call so that's ok, but notice not to call methods that would
            # lock the UI for too long.
    ...
    def _load_workspace(self):
        ...
        for row in data:
            ...
            # -1 to select the row that was just added
